...
@Autonomous(name = "Red: Get SkyStones", group = "Jan 12 Competition")
public class GetSkystoneRed extends LinearOpMode {
    private Robot robot;
        ...
        if (opModeIsActive()) {
            ...
            while (!isStopRequested() && !targetVisible) {
                ...
                if (((VuforiaTrackableDefaultListener) robot.stoneTarget.getListener()).isVisible()) {
                    ...
                    float power = 0.3f;
                    robot.Left_Front_Wheel.setPower(power * -0.57);
                    robot.Right_Front_Wheel.setPower(power * 1);
                    robot.Left_Rear_Wheel.setPower(power * 0.55);
            ...
            while (!isStopRequested() && robot.GetNormalEncoderTicks() > normalEncoderTicks + (-8.75 * 308)) {
                float power = 0.2f;
                robot.Left_Front_Wheel.setPower(power * -1);
                robot.Right_Front_Wheel.setPower(power * .9);
                robot.Left_Rear_Wheel.setPower(power);
                robot.Right_Rear_Wheel.setPower(power * -1);
            ...
            while (!((ElapsedTime2.milliseconds() >= 300) || isStopRequested())) {
                robot.RightBlockGrabber((float) 0.7);
            }
            sleep(500);
            robot.RightBlockGrabber(0);
